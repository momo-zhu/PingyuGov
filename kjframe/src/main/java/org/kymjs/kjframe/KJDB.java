...
public class KJDB {
    ...
    public void saveOrUpdate(Object entity) {
        ...
        Object id = tableInfo.getId().getValue(entity);
    ...
    private void insertContentValues(List<KeyValue> list, ContentValues cv) {
        if (list != null && cv != null) {
            for (KeyValue kv : list) {
                cv.put(kv.getKey(), kv.getValue() == null ? null : kv.getValue().toString());
            }
        } else {
    ...
    public void update(Object entity, String strWhere, String[] properties) {
        checkTableExist(entity.getClass());
        exeSqlInfo(SqlBuilder.getUpdateSqlAsSqlInfo(entity, strWhere, properties));
    }
     ...
     *
     * @param entity
     * @param strWhere :strWhere表示sql语句update xxx from xxx where后的语句，
     *                 条件为空的时候，将会更新所有的数据
     */
     ...
     *
     * @param clazz
     * @param strWhere ：strWhere表示sql语句后delete xxx from xxx where的语句，条件为空的时候
     *                 将会删除所有的数据
     */
     ...
     * 查询所有数据表
     */
    public Cursor exeSql(String formatSql, Class clazz) {
        TableInfo table = TableInfo.get(clazz);
        if (!tableIsExist(table)) {
            db.execSQL(SqlBuilder.getCreatTableSQL(clazz));
        }
        return db.rawQuery(String.format(formatSql, table.getTableName()), null);
    }
    ...
    重置table是否存在信息
     */
    public void resetTableCheck() {
        TableInfo.resetTableCheck();
    }
     ...
     *
     * @param clazz
     * @param strWhere ：strWhere表示sql语句select xxx from xxx where后的语句， 条件为空的时候查找所有数据
     */
    ...
    private void checkTableExist(Class<?> clazz) {
        if (!tableIsExist(TableInfo.get(clazz))) {
            ...
            debugSql(sql);
            db.execSQL(sql);
            List<String> sqls = SqlBuilder.getIndexTableSQL(clazz);
            for (String s : sqls) {
                debugSql(s);
                db.execSQL(s);
            }
